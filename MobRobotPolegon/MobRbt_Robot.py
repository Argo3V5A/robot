...
class Robot():
    ...
    def __init__(self, Xr, Yr, fi, SetPlg):
        ...
        self.SnsU = np.array([0.0, np.pi / 8, 2 * np.pi / 8, 3 * np.pi / 8,
                              np.pi / 2, 5 * np.pi / 8, 6 * np.pi / 8, 7 * np.pi / 8,
                              np.pi, -7 * np.pi / 8, -6 * np.pi / 8, -5 * np.pi / 8,
                              -np.pi / 2, -3 * np.pi / 8, -2 * np.pi / 8, -np.pi / 8],
        ...
        self.SnsKMrOP = np.zeros((3, 8),
                                 dtype='float32')  # Повор.мом.при об.преп.[FF,FFR, FR, RR, ZZ, LL, FL,FFL]
    ...
    def PosOrient(self):
        ...
        RP1[:, 0] = self.RP[:, 0] * np.cos(self.fi) - self.RP[:, 1] * np.sin(self.fi)
        RP1[:, 1] = self.RP[:, 0] * np.sin(self.fi) + self.RP[:, 1] * np.cos(self.fi)
    ...
    def RotT(self, screen, Xt, Yt):
        ...
        self.D = np.hypot((self.x_pos - Xt), (self.y_pos - Yt))

        # Угол цели (угол между направлением на север и направлением на цель) 
        if Xt >= self.x_pos:
            self.Bt = np.acos((self.y_pos - Yt) / self.D)
        else:
            self.Bt = 2 * np.pi - np.acos((self.y_pos - Yt) / self.D)
    ...
    def snsPr(self, screen):
        ...
        for r in range(self.SnsR.size):
            for u in range(self.SnsU.size):
                SnsX = int(self.x_pos + self.SnsR[r] * np.sin(self.fi + self.SnsU[u]))
                SnsY = int(self.y_pos - self.SnsR[r] * np.cos(self.fi + self.SnsU[u]))
    ...
    def MovT(self, screen, Xt, Yt):
        ...
        # Расстояние от робота до цели      
        Dk = np.hypot((self.x_pos - Xt), (self.y_pos - Yt))
        ...
        if self.D > 1:  # при малых D не вычисл., что бы не /0
            if Xt >= self.x_pos:
                self.Bt = np.arccos((self.y_pos - Yt) / self.D)
            else:
                self.Bt = 2 * np.pi - np.acos((self.y_pos - Yt) / self.D)
        ...
        if Atk < -np.pi:  # Приведение к диапазону -p...p
            Atk = Atk + 2 * np.pi
        if Atk > np.pi:
            Atk = Atk - 2 * np.pi
        ...
        if self.D < 3:  # ЦЕЛЬ ДОСТИГНУТА?
            ...
            if self.fi < -np.pi:  # Приведение к диапазону -p...p
                self.fi = self.fi + 2 * np.pi
            if self.fi > np.pi:
                self.fi = self.fi - 2 * np.pi
            ...
            Vrx = self.Vr * np.sin(self.fi)  # Составляющие скорости робота
            Vry = -self.Vr * np.cos(self.fi)
